...
const BasicTreeMap = () => {
  ...
  useEffect(() => {
    ...
    let group = svg.append("g").call(render, treemapFunc(data));

    console.log(treemapFunc(data));
    ...
    function render(group: any, root: any) {
      ...
      // node.append("title").text((d: any) => `${name(d)}\n${formatData(d.value)}`);
      ...
      /* node
        ...
        .data((d: any) =>
          (d === root ? name(d) : d.data.name)
            .split(/(?=[A-Z][^A-Z])/g)
            .concat(formatData(d.value))
        )
        .join("tspan")
        .attr("x", 3)
        .attr(
          "y",
          (d: any, i: any, nodes: any) =>
            // @ts-ignore
        ...
        .attr("font-weight", (d: any, i: any, nodes: any) =>
