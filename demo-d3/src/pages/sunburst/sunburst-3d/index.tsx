...
const Sunburst3D = () => {
  ...
  useEffect(() => {
    ...
    const arcData = arc()
      ...
      .endAngle((d: any) => d.x1)
      .padAngle((d: any) => Math.min((d.x1 - d.x0), 0.01))
      .padRadius(radius * 1.5)
      .innerRadius((d: any) => d.y0 * radius)
      .outerRadius((d: any) => Math.max(d.y0 * radius, d.y1 * radius - 1) - 10);
    ...
    if (nodeRef.current) {
      ...
      const addMesh = (root: any, color: any) => {
        ...
        const d = arcData(root.current) as string;
        const path = transformSVGPath(d);
